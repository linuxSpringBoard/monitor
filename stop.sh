...
addCron() {
  ...
  log "Initiating Stop script"
  stopProcess heartbeat heartbeat
  stopProcess start.py start.py
  log "Finished Stop script"
  rm -f /local/scratch/monitor-dashboard/report/report.html
  rm -f /local/scratch/monitor-dashboard/report/reportHost.html
  log "Removed old report files"
