# A simple python utility to monitor uptime of given urls // file name heartbeat.py
...
class Process:
	...
	def ping(self,firstRun=0):
		...
		if 'Connected' in output:
			...
			logging.error(self.name ,' is down with error', output)
			self.history.__setitem__('DOWN')
			return False
...
class MultiHost:
	...
	def ping(self, firstRun):
		...
		requestURL = 'http://example.com/api/v3/vservers/'+ nimbus
		...
		try:
			...
			logging.error("Something went wrong while checking health for " + nimbus)
		count = 0
		try:
			...
			logging.error("Something went wrong while parsing the JSON response for " , nimbus, " nimbus URL")
...
<i class="fa fa-heartbeat"></i> Service Monitor Dashboard</h1>
<table class="table">
	...
	<table style="WIDTH:100%" cellspacing="0" cellpadding="0" width="100%" border="0">
		<tr style="HEIGHT:49.5pt;">
			<td style="PADDING-RIGHT: 29px; PADDING-LEFT: 0in; PADDING-BOTTOM: 0in; WIDTH: 1%; PADDING-TOP: 0in; HEIGHT: 49.5pt"
				width="1%">
				<p><span><img height="66" alt=" " src="http://home.web.com/l.gif" NOSEND="1"></span></p>
			</td>
...
#         logging.debug('Opening report html for writing')
		logging.debug('Opening report html for writing: ', self.htmlTarget)
	...
	def sendMail(self, environment):
		...
		me = "LSB@gmail.com"
		you = ["LSB@gmail.com", "LSB@gmail.com"]
		...
		msg['From'] = me
		msg['To'] = ", ".join(you)
		emailText = "Please see the detailed report here - https://example.com:8000/report.html"
...
if __name__ == '__main__':
	...
	os.system('kinit -kt /var/creds/1.kt 1@google.COM')
	cookie = subprocess.run("curl -s -I -u : --negotiate https://authn.web.com/desktopsso/Login | awk '/GSSSO/ {print substr($2,0,length($2)-1)}' ",check=True,shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE,universal_newlines=True)
	...
	while True:
		...
		for (b,f) in zip(expectedTimeBefore, expectedTimeAfter):
			if currentTimeString > b and currentTimeString < f and (rep.intHealth==1 or rep.qaHealth==1 or rep.uatHealth==1 or rep.betaHealth ==1):
				...
				# Creating message container - the correct MIME type is multipart/alternative.
				msg = MIMEMultipart('alternative')
				msg['Subject'] = "Scheduled Health Report for all Services"
				msg['From'] = me
				msg['To'] = ", ".join(you)
